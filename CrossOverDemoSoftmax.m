...
accuracy= mean(numcorrect/379);

matrixA=TrainFeatures;
pt1=randi([1,2000]);
pt2=randi([2000,4096]);
[TwoPtcrossTrain]=MatrixTwoPointCrossOver(matrixA,pt1,pt2);
[OnePtcrossTrain]=MatrixCrossOver(matrixA,pt1);

matrixB=TestFeatures;
pt1=randi([1,2000]);
pt2=randi([2000,4096]);
[TwoPtcrossTest]=MatrixTwoPointCrossOver(matrixB,pt1,pt2);
[OnePtcrossTest]=MatrixCrossOver(matrixB,pt1);

net = trainSoftmaxLayer(OnePtcrossTrain,trainLabelNew);
Predict=net(OnePtcrossTest);

actual=testLabelNew;
plotconfusion(actual,Predict);
numcorrect = sum(actual==Predict);
accuracyTwoPoint =mean( numcorrect/379);
